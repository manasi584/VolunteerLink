...
async function handleGetAllTasks(req, res) {
  ...
  .populate("ngo", "name");   // Populate only the 'name' field from the NGO // Select specific fields to return
    if (!allTasks.length) {
...
async function handlePostTask(req, res) {
  try {
    ...
    const { title, description, requiredSkills, isRemote, location, deadline } =
      req.body;
    ...
    if (!title || !description || !requiredSkills || !deadline) {
      return res
        .status(400)
        .json({
          message: "Title, description, skills, and deadline are required.",
        });
    }
...
async function handleCurrentJobs(req, res) {
  try {
    ...
    if (!tasks.length) {
      return res
        .status(404)
        .json({ message: "No tasks found for this volunteer." });
    }
